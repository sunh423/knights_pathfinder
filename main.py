...
class Node:
	possible_status = ['fresh', 'path', 'open', 'closed', 'barrier', 'start', 'end']
	status_color = {'fresh': WHITE, 'open': GREEN, 'closed': RED,'barrier': BLACK, 'start': ORANGE, 'end':PURPLE, 'path':YELLOW}
	...
	def draw(self, win):
		pygame.draw.rect(win, self.status, (self.x, self.y, self.width, self.length))

	def update_neighbors(self, grid):
		self.neighbors = []
		try:
			if not grid[self.row - 1][self.col - 2].is_barrier():
				self.neighbors.append(grid[self.row - 1][self.col - 2]) # top N1
		except IndexError:
			pass
		try:
			if not grid[self.row - 2][self.col - 1].is_barrier():
				self.neighbors.append(grid[self.row - 2][self.col - 1]) # top N2
		except IndexError:
			pass
		try:
			if not grid[self.row - 2][self.col + 1].is_barrier():
				self.neighbors.append(grid[self.row - 2][self.col + 1]) # top N3
		except IndexError:
			pass
		try:
			if not grid[self.row - 1][self.col + 2].is_barrier():
				self.neighbors.append(grid[self.row - 1][self.col + 2]) # top N4
		except IndexError:
			pass
		try:
			if not grid[self.row + 1][self.col - 2].is_barrier():
				self.neighbors.append(grid[self.row + 1][self.col - 2]) # bottom N1
		except IndexError:
			pass
		try:
			if not grid[self.row + 2][self.col - 1].is_barrier():
				self.neighbors.append(grid[self.row + 2][self.col - 1]) # bottom N1
		except IndexError:
			pass
		try:
			if not grid[self.row + 2][self.col + 1].is_barrier():
...
def algorithm(draw, grid, start, end, win):      #our A* pathfinding algorithm we are going to use
	count = 0
...
def make_grid(rows, columns, width, length): #Creates nodes and a nested list for data array
	grid = []
	w_gap = width // rows
...
def draw_gridlines(win, rows, columns, width, length):      #Draws all the gridlines to separate the nodes.
	w_gap = width // rows
	l_gap = length // columns
	for i in range(rows):
		pygame.draw.line(win, GREY, (0,i * w_gap), (width, i * w_gap)) #pygame.draw.line(SURAFE, COLOR, start x/y, end x/y)
	for j in range(columns):
...
def main(win, width, length):
	...
	while run:
		if result != None:
			if result:
				popup = Text_Window(f"Shortest path was found! Number of moves: {steps}")
				popup.draw(win)
				pygame.display.update()
			else:
		...
		for event in pygame.event.get():
			...
			if pygame.mouse.get_pressed()[0] and result == None: #mouse gives a list of booleans for different mouse keys. 0 refer to left click
			...
			if pygame.mouse.get_pressed()[2] and result == None: #mouse gives a list of booleans for different mouse keys. 2 refer to right click
				pos = pygame.mouse.get_pos()
				row, col = get_clicked_pos(pos, ROWS, COLS, width, length)
				node = grid[row][col]
				node.reset()
			...
			if event.type == pygame.KEYDOWN:
				if event.key == pygame.K_SPACE and start and end and result == None:
					...
					result, steps = algorithm(lambda: draw(win, grid, ROWS, COLS, width, length), grid, start, end, win)
					draw(win, grid, ROWS, COLS, width, length)
